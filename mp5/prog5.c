 ...
 * should be initialized to 1 (to indicate the first guess) The score should be
 * initialized to -1. INPUTS: none OUTPUTS: none RETURN VALUE: none SIDE
 * EFFECTS: records the solution in the static solution variables for use by
 * make_guess, set guess_number
 */
void start_game() {
    // your code here
    for (int i = 0; i < 4; i++) {
 ...
 * 4 strings from pool). If it is valid the number of correct and incorrect
 * matches should be printed, using the following format "With guess %d, you got
 * %d perfect matches and %d misplaced matches.\n" If valid, the guess_number
 * should be incremented. If invalid, the error message "make_guess: invalid
 * guess\n" should be printed and 0 returned. For an invalid guess, the
 * guess_number is not incremented. INPUTS: guess_str -- a string consisting of
 * the guess typed by the user OUTPUTS: none RETURN VALUE: 2 if guess string is
 * valid and got all 4 perfect matches, or 1 if the guess string is valid, or 0
 * if it is invalid SIDE EFFECTS: prints (using printf) the number of matches
 * found and increments guess_number(valid guess) or an error message (invalid
 * guess) (NOTE: the output format MUST MATCH EXACTLY, check the wiki writeup)
 */
int make_guess(const char guess_str[]) {
    ...
    printf(
        ...
        guess_number, perfectMatchCount, misplacedMatchCount, score, max_score);

    guess_number++;

    return perfectMatchCount == 4 ? 2 : 1;
}
